...
static AnyKRecord all_keys[] = {
  ...
  [POWER_INDEX(MOD4)]     = { .state = ANYK_NONE },
  [POWER_INDEX(RMOD3)]    = { .state = ANYK_NONE },
...
bool process_key_internal(uint16_t keycode, keyevent_t *event, bool interrupted, uint16_t interruptedKeycode) {
  Payload payload = IS_EAGER_POWER(keycode) ? process_eager_strategy(keycode, event, interrupted, interruptedKeycode) : process_any_key(keycode, event, interrupted);
...
bool process_key_option(uint16_t keycode, keyevent_t *event, bool interrupted, uint16_t interruptedKeycode) {
  Payload payload = process_eager_strategy(keycode, event, interrupted, interruptedKeycode);
  AnyKTransition *transition = &payload.transition;
...
bool is_corner_puncs(uint16_t keycode) {
  uint16_t masked_keycode = keycode & 0xFF;
      ...
      if (interrupted) {
        if (interruptedKeycode == 0
         ...
         || interruptedKeycode == orch->key) {
          ...
          key->state = ANYK_HOLD;
        } else if (interruptedKeycode < QK_MODS) {
          if (event->pressed) {
            key->state = ANYK_HOLD;
  ...
  switch (key->state)
  {
    ...
    case ANYK_TAP:
      if (interrupted) {
        ...
        key->state = ANYK_TAPPING_UP;
        if ((orch->key & QK_RSFT) && (orch->mods & MOD_MASK_SHIFT)) {
